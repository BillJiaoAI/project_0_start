...
flash_attn_vec(const float* __restrict__ Q,
    ...
    half2 VKQ[(D_HEAD / 2) / NTHREADS_V] = {};         // [8] 输出累加器 (f16!) 必须清零
...
#pragma unroll
    ...
    // 注意: K/V 指针必须先跳到本 block 的起始 chunk (llama.cpp 用 k_VKQ_0*stride 偏移),
    // 否则 blockIdx.y>0 的块会拿 key 0.. 的分数配 key 128.. 的 mask, 结果全错!
    K_h2 += (size_t)blockIdx.y * NTHREADS * (D_HEAD / 2);
    V_h2 += (size_t)blockIdx.y * NTHREADS * (D_HEAD / 2);
